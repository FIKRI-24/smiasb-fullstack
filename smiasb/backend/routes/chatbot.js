...
const { pool, isPostgres, dbPlaceholder } = require('../config/database');
...
async function ensureChatbotCacheTable() {
  ...
  try {
    if (isPostgres) {
      ...
      await pool.execute('CREATE INDEX IF NOT EXISTS idx_chatbot_cache_expires ON chatbot_cache (expires_at)');
      await pool.execute('CREATE INDEX IF NOT EXISTS idx_chatbot_cache_last_hit ON chatbot_cache (last_hit_at)');
    } else {
      await pool.execute(
        `CREATE TABLE IF NOT EXISTS chatbot_cache (
...
async function getCachedGeminiResponse(normalizedPrompt) {
  ...
  try {
    ...
    const isActiveVal = isPostgres ? 'TRUE' : '1';
    const result = await pool.execute(
      `SELECT id, balasan
       FROM chatbot_cache
       WHERE normalized_prompt_hash = ${dbPlaceholder(1)}
         AND normalized_prompt = ${dbPlaceholder(2)}
         AND is_active = ${isActiveVal}
    ...
    if (isPostgres) {
      await pool.execute(
        `INSERT INTO chatbot_cache
         ...
         VALUES ($1, $2, $3, $4, NOW() + INTERVAL '90 days', NOW(), NOW())
         ON CONFLICT (normalized_prompt_hash) DO UPDATE SET
          normalized_prompt = EXCLUDED.normalized_prompt,
          balasan = EXCLUDED.balasan,
...
async function getChatHistoryColumns() {
  ...
  const sql = isPostgres
    ? `SELECT column_name
       ...
       WHERE table_schema = current_schema() AND table_name = 'chat_history'`
    : `SELECT COLUMN_NAME
       FROM INFORMATION_SCHEMA.COLUMNS
  ...
  chatHistoryColumnsCache = new Set(
...
async function resolveChatInstrumenId(user, rawInstrumenId) {
  ...
  const result = await pool.execute(
    `SELECT *
     FROM instrumen
     WHERE id_sekolah = ${dbPlaceholder(1)}
       AND kelas = ${dbPlaceholder(2)}
       AND status = 'aktif'
...
router.post('/send', authenticate, async (req, res) => {
  ...
  try {
    ...
    const chatInstrumenId = hasInstrumenColumn
      ...
      : null;
    const isError = isChatbotErrorResponse(balasan) ? (isPostgres ? true : 1) : (isPostgres ? false : 0);

    if (hasInstrumenColumn && hasIsErrorColumn) {
      await pool.execute(
        `INSERT INTO chat_history (user_id, instrumen_id, pesan, balasan, is_error, created_at) VALUES (${dbPlaceholder(1)}, ${dbPlaceholder(2)}, ${dbPlaceholder(3)}, ${dbPlaceholder(4)}, ${dbPlaceholder(5)}, NOW())`,
        [req.user.id, chatInstrumenId, pesanAsli, balasan, isError]
      );
    } else if (hasInstrumenColumn) {
      await pool.execute(
        `INSERT INTO chat_history (user_id, instrumen_id, pesan, balasan, created_at) VALUES (${dbPlaceholder(1)}, ${dbPlaceholder(2)}, ${dbPlaceholder(3)}, ${dbPlaceholder(4)}, NOW())`,
        [req.user.id, chatInstrumenId, pesanAsli, balasan]
      );
    } else {
      await pool.execute(
        `INSERT INTO chat_history (user_id, pesan, balasan, created_at) VALUES (${dbPlaceholder(1)}, ${dbPlaceholder(2)}, ${dbPlaceholder(3)}, NOW())`,
...
router.get('/history', authenticate, async (req, res) => {
  try {
    ...
    const limitPlaceholder = isPostgres ? `${dbPlaceholder(2)}::int` : dbPlaceholder(2);
    const result = await pool.execute(
...
router.delete('/history', authenticate, async (req, res) => {
  try {
    const result = await pool.execute(
      `DELETE FROM chat_history WHERE user_id = ${dbPlaceholder(1)}`,
      [req.user.id]
    );

    return res.json({
      success: true,
      message: `Riwayat chat berhasil dihapus (${resultRowCount(result)} pesan).`
    });
